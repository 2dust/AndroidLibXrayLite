...
import (
	...
	"github.com/dnscrypt/dnscrypt-proxy/lib"
...
type App struct {
	...
	quit  chan struct{}
	proxy *lib.Proxy
	flags *lib.ConfigFlags
}

func StartDnscryptProxy(path string) {
	quit = make(chan bool)
	go func() {
		for {
			select {
...
func helper(path string) {
	...
	app.proxy = lib.NewProxy()
	_ = lib.ServiceManagerStartNotify()
	if len(*svcFlag) != 0 {
		if svc == nil {
...
func (app *App) AppMain() {
	if err := lib.ConfigLoad(app.proxy, app.flags); err != nil {
		dlog.Fatal(err)
	}
	if err := lib.PidFileCreate(); err != nil {
