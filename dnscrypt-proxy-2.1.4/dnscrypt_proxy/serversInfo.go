...
func (serversInfo *ServersInfo) refreshServer(proxy *Proxy, name string, stamp stamps.ServerStamp) error {
	...
	if name != newServer.Name {
		log.Fatalf("[%s] != [%s]", name, newServer.Name)
	}
...
func (serversInfo *ServersInfo) refresh(proxy *Proxy) (int, error) {
	...
	innerLen := len(inner)
	if innerLen > 1 {
		log.Println("Sorted latencies:")
		for i := 0; i < innerLen; i++ {
			log.Printf("- %5dms %s", inner[i].initialRtt, inner[i].Name)
		}
	}
	if innerLen > 0 {
		log.Printf("Server with the lowest initial latency: %s (rtt: %dms)", inner[0].Name, inner[0].initialRtt)
	}
	serversInfo.Unlock()
...
func (serversInfo *ServersInfo) getOne() *ServerInfo {
	...
	log.Printf("Using candidate [%s] RTT: %d", (*serverInfo).Name, int((*serverInfo).rtt.Value()))
	serversInfo.Unlock()
...
func route(proxy *Proxy, name string, serverProto stamps.StampProtoType) (*Relay, error) {
	...
	if err != nil {
		log.Printf("Server [%v]'s protocol doesn't support anonymization", name)
		return nil, nil
	}
	...
	case stamps.StampProtoTypeDNSCrypt, stamps.StampProtoTypeDNSCryptRelay:
		...
		log.Printf("Anonymizing queries for [%v] via [%v]", name, relayName)
		return &Relay{
...
func fetchDNSCryptServerInfo(proxy *Proxy, name string, stamp stamps.ServerStamp, isNew bool) (ServerInfo, error) {
	if len(stamp.ServerPk) != ed25519.PublicKeySize {
		...
		if err != nil || len(serverPk) != ed25519.PublicKeySize {
			log.Fatalf("Unsupported public key for [%s]: [%s]", name, stamp.ServerPk)
		}
		log.Printf("Public key [%s] shouldn't be hex-encoded any more", string(stamp.ServerPk))
	...
	for _, buggyServerName := range proxy.serversBlockingFragments {
		if buggyServerName == name {
			...
			log.Printf("Known bug in [%v]: fragmented questions over UDP are blocked", name)
			break
		}
	}
...
func fetchDoHServerInfo(proxy *Proxy, name string, stamp stamps.ServerStamp, isNew bool) (ServerInfo, error) {
	...
	if _, _, _, _, err := proxy.xTransport.DoHQuery(useGet, url, body, proxy.timeout); err != nil {
		...
		log.Printf("Server [%s] doesn't appear to support POST; falling back to GET requests", name)
	}
	body = dohNXTestPacket(0xcafe)
	serverResponse, _, tls, rtt, err := proxy.xTransport.DoHQuery(useGet, url, body, proxy.timeout)
	if err != nil {
		log.Printf("[%s] [%s]: %v", name, url, err)
		return ServerInfo{}, err
	}
	...
	if msg.Rcode != dns.RcodeNameError {
		log.Printf("[%s] may be a lying resolver", name)
	}
	...
	if strings.HasPrefix(protocol, "http/1.") {
		log.Printf("[%s] does not support HTTP/2 nor HTTP/3", name)
	}
	log.Printf("[%s] TLS version: %x - Protocol: %v - Cipher suite: %v", name, tls.Version, protocol, tls.CipherSuite)
	...
	for _, cert := range tls.PeerCertificates {
		...
		if showCerts {
			log.Printf("Advertised cert: [%s] [%x]", cert.Subject, h)
		} else {
			log.Printf("Advertised cert: [%s] [%x]", cert.Subject, h)
		}
	...
	if !found && len(stamp.Hashes) > 0 {
		log.Printf("[%s] Certificate hash [%x] not found", name, wantedHash)
...
func _fetchODoHTargetInfo(proxy *Proxy, name string, stamp stamps.ServerStamp, isNew bool) (ServerInfo, error) {
	...
	if relay == nil {
		...
		if relay.ODoH == nil {
			log.Printf("Wrong relay type defined for [%v] - ODoH servers require an ODoH relay", name)
			return ServerInfo{}, errors.New("Wrong ODoH relay type")
		}
	}

	log.Printf("Pausing after ODoH configuration retrieval")
	delay := time.Duration(rand.Intn(5*1000)) * time.Millisecond
	clocksmith.Sleep(time.Duration(delay))
	log.Printf("Pausing done")
	...
	for _, odohTargetConfig := range odohTargetConfigs {
		...
		if err != nil {
			log.Printf("Unable to decrypt response from [%v]: [%v]", name, err)
			continue
		}
		...
		if strings.HasPrefix(protocol, "http/1.") {
			log.Printf("[%s] does not support HTTP/2", name)
		}
		log.Printf(
		...
		for _, cert := range tls.PeerCertificates {
			h := sha256.Sum256(cert.RawTBSCertificate)
			if showCerts {
				log.Printf("Advertised relay cert: [%s] [%x]", cert.Subject, h)
			} else {
				log.Printf("Advertised relay cert: [%s] [%x]", cert.Subject, h)
			}
			for _, hash := range stamp.Hashes {
		...
		xrtt := int(rtt.Nanoseconds() / 1000000)
		if isNew {
			log.Printf("[%s] OK (ODoH) - rtt: %dms", name, xrtt)
		} else {
			log.Printf("[%s] OK (ODoH) - rtt: %dms", name, xrtt)
		}
		return ServerInfo{
...
func fetchODoHTargetInfo(proxy *Proxy, name string, stamp stamps.ServerStamp, isNew bool) (ServerInfo, error) {
	...
	for i := 0; i < 3; i += 1 {
		...
		log.Printf("Trying to fetch the [%v] configuration again", name)
	}
	return serverInfo, err
}
