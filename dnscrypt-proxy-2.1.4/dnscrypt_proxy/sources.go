...
func (source *Source) fetchFromCache(now time.Time) (delay time.Duration, err error) {
	...
	if elapsed := now.Sub(fi.ModTime()); elapsed < source.cacheTTL {
		...
		log.Printf("Source [%s] cache file [%s] is still fresh, next update: %v", source.name, source.cacheFile, delay)
	} else {
		log.Printf("Source [%s] cache file [%s] needs to be refreshed", source.name, source.cacheFile)
	}
	return
}
...
func (source *Source) parseURLs(urls []string) {
	for _, urlStr := range urls {
		if srcURL, err := url.Parse(urlStr); err != nil {
			log.Printf("Source [%s] failed to parse URL [%s]", source.name, urlStr)
		} else {
...
func (source *Source) fetchWithCache(xTransport *XTransport, now time.Time) (delay time.Duration, err error) {
	if delay, err = source.fetchFromCache(now); err != nil {
		if len(source.urls) == 0 {
			log.Printf("Source [%s] cache file [%s] not present and no valid URL", source.name, source.cacheFile)
			return
		}
		log.Printf("Source [%s] cache file [%s] not present", source.name, source.cacheFile)
	}
	if len(source.urls) > 0 {
		defer func() {
	...
	for _, srcURL := range source.urls {
		log.Printf("Source [%s] loading from URL [%s]", source.name, srcURL)
		sigURL := &url.URL{}
		*sigURL = *srcURL // deep copy to avoid parsing twice
		sigURL.Path += ".minisig"
		if bin, err = fetchFromURL(xTransport, srcURL); err != nil {
			log.Printf("Source [%s] failed to download from URL [%s]", source.name, srcURL)
			continue
		}
		if sig, err = fetchFromURL(xTransport, sigURL); err != nil {
			log.Printf("Source [%s] failed to download signature from URL [%s]", source.name, sigURL)
			continue
		}
		if err = source.checkSignature(bin, sig); err == nil {
			break // valid signature
		} // above err check inverted to make use of implicit continue
		log.Printf("Source [%s] failed signature check using URL [%s]", source.name, srcURL)
	}
	if err != nil {
		return
	}
...
func NewSource(
	...
	if _, err = source.fetchWithCache(xTransport, timeNow()); err == nil {
		log.Printf("Source [%s] loaded", name)
	}
	return
}
...
func PrefetchSources(xTransport *XTransport, sources []*Source) time.Duration {
	...
	for _, source := range sources {
		...
		log.Printf("Prefetching [%s]", source.name)
		if delay, err := source.fetchWithCache(xTransport, now); err != nil {
			log.Printf("Prefetching [%s] failed: %v, will retry in %v", source.name, err, interval)
		} else {
			log.Printf("Prefetching [%s] succeeded, next update: %v", source.name, delay)
...
func (source *Source) Parse() ([]RegisteredServer, error) {
	...
	log.Fatal("Unexpected source format")
...
func (source *Source) parseV2() ([]RegisteredServer, error) {
	...
	for _, part := range parts {
		...
		log.Printf("Registered [%s] with stamp [%s]", name, stamp.String())
